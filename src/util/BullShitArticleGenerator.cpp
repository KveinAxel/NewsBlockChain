#include "BullShitArticleGenerator.h"
#include "cstdlib"
#include "ctime"
...
std::string giveMeTheQuotes() {
    srand((unsigned) time(NULL));
    std::string res = quotes[rand() % quotes->size()];
    int pos = res.find("曾经说过");
    while (pos != -1) {
        res.replace(pos, std::string("曾经说过").length(), before[rand() % before->size()]);
        pos = res.find("曾经说过");
    }
    pos = res.find("这不禁令我深思");
    while (pos != -1) {
        res.replace(pos, std::string("这不禁令我深思").length(), after[rand() % before->size()]);
        pos = res.find("这不禁令我深思");
    }
    return res;
}
...
std::string BullShitArticleGenerator::generator(unsigned seed) {
    ...
    for (int i = 0; i < para; i++) {
        int textNum = rand() % 5 + 4;
        for (int j = 0; j < textNum; j++) {
            int a = rand() % 28;
            contain.append(text[a]);
            if (j % 2 == 0) {
                int b = rand() % 100;
                contain.append(quotes[b]);
            }
        }
        int c = rand() % 5;
        contain.append(after[c]);
        contain.append("\n");
    }
    int pos;
    pos = contain.find("x");
    while (pos != -1) {
        contain.replace(pos, std::string("x").length(), xx);
        pos = contain.find("x");
    }
    return contain;
}