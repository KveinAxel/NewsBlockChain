...
std::string giveMeTheQuotes(){
    ...
    std::string xx = quotes[rand()%quotes->size()];
    int pos = xx.find("曾经说过");
    while(pos != -1){
        xx.replace(pos,std::string("曾经说过").length(),before[rand()%before->size()]);
        pos = xx.find("曾经说过");
    }
    pos = xx.find("这不禁令我深思");
    while(pos != -1){
        xx.replace(pos,std::string("这不禁令我深思").length(),after[rand()%before->size()]);
        pos = xx.find("这不禁令我深思");
    }
    return xx;
}
