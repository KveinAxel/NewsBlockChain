...
public:
    ...
    QList<int>  modifyCheckLocal(const QList<std::string>& article,string& keyOfResponse);//要选定区块的才能检测，这个函数就离谱
    ...
    QPair<QJsonObject,bool> responseBlock(string& keyOfResponse);//返回请求的区块，如果blockChainHash内存中没有，则数据库中调用block先到内存，再
    ...
    */
    ...
    static bool containLegalBlock(string& hashKey);
};
